...
# -------------------------------------------------
DeepseekV32MoEGate = DeepseekV3TopkRouter
class _DeepseekV32MoEGate(nn.Module):
...
DeepseekV32RotaryEmbedding = DeepseekV3RotaryEmbedding
...
# from .modular_deepseek_v32 import DeepseekV32Attention

class _DeepseekV32Attention(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.register_buffer(
            "kv_cache", torch.zeros(config.max_batch_size, config.max_seq_len, self.config.kv_lora_rank), persistent=False
        )
        self.register_buffer(
            "pe_cache", torch.zeros(config.max_batch_size, config.max_seq_len, self.config.qk_rope_head_dim), persistent=False
        )
