...
class OoTQwen3MoeExperts(torch.nn.ModuleList):
    ...
    def __init__(self, config):
        ...
        if self.config._experts_implementation == "grouped_mm":
            self.post_porcess_for_grouped_mm()
            self.forward = self.group_mm_forward
        else:
            self.forward = self.eager_forward

    def post_porcess_for_grouped_mm(self):
        self.gate_up_proj = nn.Parameter(torch.empty(self.num_experts, 2 * self.intermediate_dim, self.hidden_dim))
        self.down_proj = nn.Parameter(torch.empty(self.num_experts, self.hidden_dim, self.intermediate_dim))
        # move existing weights to these parameters
        with torch.no_grad():
            for expert_idx in range(self.num_experts):
                self.gate_up_proj[expert_idx] = torch.cat(
                    [
                        self.gate_proj.weight
                        for self.gate_proj in [self[expert_idx].gate_proj, self[expert_idx].up_proj]
                    ],
                    dim=0,
                )
                self.down_proj[expert_idx] = self[expert_idx].down_proj.weight
        # delete existing modules to save memory
        for expert_idx in range(self.num_experts):
            del self[expert_idx].gate_proj
            del self[expert_idx].up_proj
            del self[expert_idx].down_proj
    ...
    def group_mm_forward(
        ...
        # Call with grouped mm implementation
        pass
